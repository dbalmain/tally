...
| `R`                 | Refresh imports (Todo → Uncategorised) — reruns pull/import scripts on a background store connection (the same flow as the automatic startup refresh, which now also goes through `App::request_refresh`); the "Refreshing..." tab-bar indicator shows while it runs, replaced by a green added/skipped/files summary on completion. Ignored while a refresh is already in flight                                                                                                                                                                |
