...
- **Non-blocking outcomes use the tab-bar status, not modals** — the tab bar's
  right side is a transient status area (`draw_tabs` in `ui.rs`): a DarkGray
  indicator while a background job is in flight (`app.active_job` →
  "Refreshing..." / "Classifying..." / "Reindexing..."), and via
  `app.show_status(msg)`, a green one-line result message shown for 5 s. Only
  one of refresh / classify / reindex may run at a time (WAL one-writer);
  conflicts are reported uniformly as "<job> in progress". It never captures
  input and needs no dismissal. Use it for fire-and-forget results (e.g. the `C`
  classification summary); reserve modals for flows that need a decision or
  acknowledgement.
...
| `Ctrl-G`            | Reindex full-text search (rebuild `transactions_fts` from every transaction via `build_searchable_text`). Global on every tab; **hidden from the footer**, listed in the `?` keybind popover. Claims the shared background-job slot (`BackgroundJob::Reindex`); "Reindexing..." tab-bar indicator while it runs, green "Reindexed N transactions" on completion. Blocked with "… in progress" if another background job holds the slot (WAL one-writer). Ctrl-G (not Ctrl-I) because legacy terminals and tmux deliver Ctrl-I as the bare Tab byte |
...
Shared checkbox list for three flows: the similar-transactions offer after a
manual `c` categorisation, bulk categorise-all (`C`), and accept-all (`A` on AI
Review / Transfer Review). Title reflects the action (apply category path,
accept categories, or accept transfers) and the selected count. Rows are
pre-selected; the user can scroll and untick before confirming. Transaction rows
show date/description/amount (plus a score column only for the similar-
transactions flow); transfer rows mirror Transfer Review columns.
...
- **DB search (`/`)** pushes filters to SQL: `date:2024-01..2024-06`,
  ...
  after any `/`, e.g. `category:/Groceries`; `|` for OR), `confidence:<60` (AI
  confidence as a **percentage**, `%` suffix optional, alias `conf:`; `>80`,
  `40..60`, `..60`, `40..`; a bare `confidence:85` matches the rounding bucket
  ...
  words are FTS5 full-text search (`coffee OR tea`, `"exact phrase"`, `coff*`);
  `/pattern/i` is regex. End with `~` to switch to fuzzy mode keeping the DB
  filters.
