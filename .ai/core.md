...
- **Non-blocking outcomes use the tab-bar status, not modals** — the tab bar's
  ...
  input and needs no dismissal. Use it for fire-and-forget results (e.g. the
  `C` classification summary); reserve modals for flows that need a decision
  or acknowledgement.
...
pre-selected; the user can scroll and untick before confirming. Transaction
rows show date/description/amount (plus a score column only for the similar-
